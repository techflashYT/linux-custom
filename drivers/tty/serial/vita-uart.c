...
static void
vita_uart_set_termios(struct uart_port *port, struct ktermios *new,
...
static void vita_uart_console_putchar(struct uart_port *port, unsigned char chr)
{
	while (!vita_uart_tx_ready(port))
		cpu_relax();

	vita_uart_write32(port, chr, VITA_UART_WRITE_FIFO);
...
static void vita_early_putchar(struct uart_port *port, unsigned char chr)
{
	while (!vita_uart_tx_ready(port))
		cpu_relax();

	writel(chr, port->membase + VITA_UART_STATUS);
